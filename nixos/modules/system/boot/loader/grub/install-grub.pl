...
# /data/data/com.termux/files/nix/store.  If not, then all kernels and initrds must be copied to
# the bootPath.
if (stat($bootPath)->dev != stat("/data/data/com.termux/files/nix/store")->dev) {
    $copyKernels = 1;
}
...
sub GetFs {
    ...
    foreach my $fs (read_file("/proc/self/mountinfo")) {
        ...
        # Skip the bind-mount on /data/data/com.termux/files/nix/store.
        next if $mountPoint eq "/data/data/com.termux/files/nix/store" && (grep { $_ eq "rw" } @superOptions);
...
sub copyToKernelsDir {
    my ($path) = @_;
    return $grubStore->path . substr($path, length("/data/data/com.termux/files/nix/store")) unless $copyKernels;
...
addProfile "/data/data/com.termux/files/nix/var/nix/profiles/system", "@distroName@ - All configurations";

for my $profile (glob "/data/data/com.termux/files/nix/var/nix/profiles/system-profiles/*") {
