...
- In NixOS virtual machines (QEMU), the `virtualisation` module has been updated with new options:
  ...
  In addition, the default [`msize`](options.html#opt-virtualisation.msize) parameter in 9P filesystems (including /data/data/com.termux/files/nix/store and all shared directories) has been increased to 16K for improved performance.
...
- The [networking.wireless](options.html#opt-networking.wireless.enable) module (based on wpa_supplicant) has been heavily reworked, solving a number of issues and adding useful features:
  ...
  - Secrets like pre-shared keys and passwords can now be handled safely, meaning without including them in a world-readable file (`wpa_supplicant.conf` under /data/data/com.termux/files/nix/store).
